...
- FastAPI + SQLite 单体后端，240 条带分析模式的可重复 Demo 数据
- 统一 OpenAI-compatible 模型 Provider，未配置模型时提供可靠本地回退
- 6 个独立 LangChain Tool 与 Tool Registry
- LangGraph 路由、数据查询、RAG、综合分析工作流
- SSE 实时业务 Trace、回答与可追溯 Sources
- 本地 RAG 索引，支持 Markdown、TXT、PDF、DOCX
...
支持 `.md`、`.txt`、`.pdf`、`.docx`。本地索引保存在 `backend/data/rag_index.json`，检索结果包含 `document_name`、`chunk`、`score`。这是轻量 Competition Mode 方案，后续可替换 `rag/vector_store.py` 接入 Embedding Vector Store。
